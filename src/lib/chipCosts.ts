 ...
 * and unknown rarities have no seed row. Returns null too if any step in
 * the range has an unknown (null) cost.
 */
 ...
 * between). `to` at or below `from` returns 0. Returns null for ICONIC /
 * unknown rarity, or when any step in the range has an unknown cost.
 */
export function chipsBetween(rarity: Rarity | undefined, from: Tier, to: Tier): number | null {
  ...
  for (let i = fromIdx; i < toIdx; i++) {
    ...
    if (step === null || step === undefined) return null; // unknown cost in range
    total += step;
  }
  return total;
}
